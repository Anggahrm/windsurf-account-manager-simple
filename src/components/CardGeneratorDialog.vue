    ...
    <div class="card-generator-content">
        ...
        <div class="bin-add-form">
          <div class="form-row">
            <el-input
              ...
              placeholder="BINprefix (e.g.: 411111)"
              size="default"
              class="bin-input"
            />
          </div>
          <div class="form-row">
            ...
            <el-button type="primary" size="default" :icon="Plus" @click="addBin">Add</el-button>
          </div>
        </div>
          ...
          <TransitionGroup name="bin-list">
            ...
            >
              ...
              <div class="bin-content">
                ...
                <div class="bin-meta">
                  <span class="bin-digits"><el-icon><Postcard /></el-icon>{{ getCardDigits(bin.type) }}</span>
                  <span class="bin-cvc"><el-icon><Key /></el-icon>{{ getCvcDigits(bin.type) }}</span>
                </div>
              </div>
              <el-button
                type="danger"
      ...
      <div class="generator-panel">
        ...
        <div class="generator-options">
          ...
          <div class="option-row">
            <span class="option-label">Expiry Month</span>
        ...
        <div class="generated-cards">
          <div class="cards-header" v-if="generatedCards.length > 0">
            <div class="cards-header-left">
              ...
              <span>Generated <strong>{{ generatedCards.length }}</strong>  cards</span>
            </div>
          ...
          <div class="cards-list">
            ...
            <div v-if="generatedCards.length === 0" class="empty-state cards-empty">
              ...
              <p>Select BIN after clicking "Generate Cards"</p>
            </div>
          </div>
        </div>
      </div>
...
function addBin() {
  if (!newBin.value.prefix) {
    ElMessage.warning('Please enterBINprefix');
    return;
  }
  if (!/^\d{4,8}$/.test(newBin.value.prefix)) {
    ElMessage.warning('BINprefixshouldas4-8-digit number');
    return;
  }
  if (binList.value.some(b => b.prefix === newBin.value.prefix)) {
    ElMessage.warning('theBINAlready exists');
    return;
  }
  binList.value.push({ ...newBin.value });
  newBin.value = { prefix: '', type: 'visa' };
  saveBinList();
  ElMessage.success('BINAdded successfully');
}
...
// Luhnalgorithmgeneratevalidate
// note：calculatevalidatewhen，PartialCard Numbermostright边countcharacterincompleteCard Numberinis倒count#2，shouldthe加倍
function luhnChecksum(cardNumber: string): number {
  let sum = 0;
  let shouldDouble = true; // fromtruestart，becausePartialCard Numbermostright边countcharacterincompleteCard Numberinneed加倍
...
function generateCards() {
  if (selectedBinIndex.value === null) {
    ElMessage.warning('please firstselectoneBIN');
    return;
  }
...
.card-generator-content {
  display: flex;
  height: 620px;
  background: #F9FAFB; /* 极淡graybackground */
}

/* left sideBINManagement */
.bin-manager {
  width: 320px;
