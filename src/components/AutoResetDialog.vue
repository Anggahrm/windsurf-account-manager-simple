                      ...
                      >
                        <div class="group-option">
                          ...
                          <span class="group-stats">
                            <el-tag type="primary" size="small">primary{{ getGroupStats(group).masters }}</el-tag>
              ...
              <div class="card-header">
                ...
                <div class="header-actions">
                  <el-input
                    ...
                    placeholder="Search accounts/Remark"
                    :prefix-icon="Search"
                    clearable
                    size="small"
                    style="width: 160px;"
        ...
        <div class="tab-content">
          <el-card shadow="never">
            ...
            <el-table :data="records" v-loading="recordsLoading" empty-text="No Reset Records">
              ...
              <el-table-column label="ResetTime" width="160">
                <template #default="{ row }">
...
async function loadConfigs() {
  ...
  try {
    ...
    console.error('LoadingAuto Resetconfigfailed:', error);
  } finally {
    loading.value = false;
  }
}
...
async function loadRecords() {
  ...
  try {
    ...
    console.error('LoadingResetrecordfailed:', error);
  } finally {
...
async function loadStats() {
  ...
  try {
    ...
    console.error('LoadingStatisticsdatafailed:', error);
  } finally {
...
async function executeCheck(configId: string) {
  try {
    ...
    console.error('Auto ResetCheck failed:', error);
  }
}

// fetchTarget Email/Group name
...
// judgewhether it isprimaryAccount（hasTeamAccount）
function isMasterAccount(account: any): boolean {
  // use is_team_owner fieldjudge，thefieldinLogin/Refreshwhenvia API fetch
  return account.is_team_owner === true;
}

// fetchGroupStatistics（primaryAccount/Team MembersCount）
function getGroupStats(group: string): { masters: number; members: number } {
  ...
  // primaryAccount：hasTeamPlanAccount
  const masters = groupAccounts.filter(a => isMasterAccount(a)).length;
  // Team Members：normalAccount
  const members = groupAccounts.length - masters;
  return { masters, members };
}

// fetchGroupTags（used forunderpulloptiondisplay）
function getGroupLabel(group: string): string {
  const stats = getGroupStats(group);
  return `${group} (primary${stats.masters}/Member${stats.members})`;
}
...
// targettypechangewhenClearselect
...
async function handleAddConfig() {
  if (!newConfig.value.targetId) {
    ElMessage.warning('Please selecttarget');
    return;
  }
...
async function handleCheckNow(config: AutoResetConfig) {
  ...
  try {
    ...
    if (result.reset_count > 0) {
      ElMessage.success(`Reset ${result.reset_count}  accounts credits`);
      await accountsStore.loadAccounts();
    } else {
      ElMessage.info('checkDone，noneedReset');
    }
...
async function handleCheckAll() {
  ...
  try {
    ...
    if (totalReset > 0) {
      ElMessage.success(`checkDone，totalReset ${totalReset}  accounts`);
      await accountsStore.loadAccounts();
    } else {
      ElMessage.info('checkDone，noneedReset');
    }
...
async function handleResetNow(config: AutoResetConfig) {
  ...
  try {
    ...
    if (result.reset_count > 0) {
      ...
      ElMessage.info('no availableResetAccount');
    }
...
async function handleResetAll() {
  ...
  try {
    ...
    if (totalReset > 0) {
      ElMessage.success(`ResetDone，totalReset ${totalReset}  accounts`);
      await accountsStore.loadAccounts();
    } else {
      ElMessage.info('no availableResetAccount');
    }
...
async function handleSaveEdit() {
  ...
  try {
    ...
    ElMessage.success('modifysuccessful');
    showEditDialog.value = false;
    editingConfig.value = null;
    await loadConfigs();
  } catch (error) {
    ElMessage.error(`modifyfailed: ${error}`);
  }
}
...
async function handleDeleteConfig(config: AutoResetConfig) {
  try {
    await ElMessageBox.confirm(
      `Confirmneed toDelete "${getTargetName(config)}" Auto Resetrules?？`,
      'Confirm Delete',
      { type: 'warning' }
    );
    ...
    ElMessage.success('Deletesuccessful');
    await loadConfigs();
  } catch (error) {
    if (error !== 'cancel') {
...
async function handleClearRecords() {
  try {
    await ElMessageBox.confirm(
      'Confirmneed toClearallResetrecord?？thisOperationnotcanrestore。',
      'ConfirmClear',
      { type: 'warning' }
    );
    
    await invoke('clear_reset_records');
    ElMessage.success('Clearsuccessful');
    await loadRecords();
    await loadStats();
  } catch (error) {
    if (error !== 'cancel') {
      ElMessage.error(`Clearfailed: ${error}`);
    }
  }
}
...
// on component unmountCleartimer
onUnmounted(() => {
  timerMap.value.forEach(timer => clearInterval(timer));
  timerMap.value.clear();
});

// exposemethod供externalcall
defineExpose({
  loadConfigs,
  setupTimers,
});
