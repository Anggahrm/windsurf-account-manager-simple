<template>
  <el-dialog
    v-model="visible"
    title="accountsingleandSubscription"
    width="800px"
    ...
    <div v-else-if="billingData" class="billing-content">
        ...
        <div class="sub-header">
          <div class="plan-info">
            ...
            <div class="plan-status">
              <el-tag v-if="billingData.on_trial" type="warning" effect="dark" round size="small">trialperiod</el-tag>
              ...
              <el-tag v-if="billingData.cancel_at_period_end" type="danger" effect="dark" round size="small">will于thisperiodendafterCancel</el-tag>
            </div>
          </div>
          <div class="sub-price" v-if="billingData.plan_unit_amount">
            <span class="currency">$</span>
            <span class="amount">{{ billingData.plan_unit_amount.toFixed(2) }}</span>
            <span class="unit" v-if="billingData.sub_interval"> / {{ billingData.sub_interval === 'yearly' ? '年' : 'month' }}</span>
          </div>
        </div>
          ...
          <div class="card-content">
            ...
            <div class="quota-tags">
              <el-tag size="small" type="info" effect="plain" v-if="billingData.base_quota">
                basic: {{ formatQuota(billingData.base_quota) }}
              </el-tag>
          ...
          <div class="card-content">
            ...
            <div class="invoice-link" v-if="billingData.invoice_url">
              <el-link type="primary" :href="billingData.invoice_url" target="_blank">
                <el-icon><Link /></el-icon> Viewmostnearsend票
              </el-link>
            </div>
          </div>
        </div>
      ...
      <div class="alerts-container" v-if="billingData.failed_payment_message || billingData.top_up_error || isApproachingCacheLimit()">
         <el-alert
          ...
          :title="`paymentfailed: ${billingData.failed_payment_message}`"
          type="error"
          :closable="false"
          show-icon
          class="mb-10"
        ...
        <el-alert
          ...
          :title="`RechargeError: ${billingData.top_up_error}`"
          type="warning"
          :closable="false"
          show-icon
          class="mb-10"
        />
         <el-alert 
          ...
          :title="`note：Cache Usageratealready 达${getCacheUsagePercentage()}%`"
          type="warning"
          :closable="false"
          show-icon
        />
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button type="primary" @click="copyToClipboard" v-if="billingData">
          <el-icon><CopyDocument /></el-icon> Copydata
        </el-button>
      </div>
    </template>
  </el-dialog>
...
function formatPlanName(name: string) {
  const names: Record<string, string> = {
    ...
    'enterprise_self_serve': 'Enterprise Enterpriseself-serviceversion',
    'trial': 'Trial Trial',
    'free': 'Free Free',
